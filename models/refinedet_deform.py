...
from models.deformableConv.dcn_v2 import dcn_v2_conv, DCNv2, DCN
from models.deformableConv.dcn_v2 import dcn_v2_pooling, DCNv2Pooling, DCNPooling
...
class RefineDet(nn.Module):
    def __init__(self, phase: str, size: int, base: List[nn.Module], extras: List[nn.Module],
        ...
        self.conv_sources_offset0 = nn.Conv2d(512, 2*3*3, kernel_size=3, stride=1, padding=1)
        self.conv_sources_offset1 = nn.Conv2d(512, 2*3*3, kernel_size=3, stride=1, padding=1)
        self.conv_sources_offset2 = nn.Conv2d(1024, 2*3*3, kernel_size=3, stride=1, padding=1)
        self.conv_sources_offset3 = nn.Conv2d(512, 2*3*3, kernel_size=3, stride=1, padding=1)
        
        self.conv_sources_modu0 = nn.Conv2d(512, 3*3, kernel_size=3, stride=1, padding=1)
        self.conv_sources_modu1 = nn.Conv2d(512, 3*3, kernel_size=3, stride=1, padding=1)
        self.conv_sources_modu2 = nn.Conv2d(1024, 3*3, kernel_size=3, stride=1, padding=1)
        self.conv_sources_modu3 = nn.Conv2d(512, 3*3, kernel_size=3, stride=1, padding=1)
        
        self.sources_dcn0 = DCNv2(512, 512, kernel_size=3, stride=1, padding=1).cuda()
        self.sources_dcn1 = DCNv2(512, 512, kernel_size=3, stride=1, padding=1).cuda()
        self.sources_dcn2 = DCNv2(1024, 1024, kernel_size=3, stride=1, padding=1).cuda()
        self.sources_dcn3 = DCNv2(512, 512, kernel_size=3, stride=1, padding=1).cuda()

        self.conv_tcb_offset0 = nn.Conv2d(256, 2*3*3, kernel_size=3, stride=1, padding=1)
        self.conv_tcb_offset1 = nn.Conv2d(256, 2*3*3, kernel_size=3, stride=1, padding=1)
        self.conv_tcb_offset2 = nn.Conv2d(256, 2*3*3, kernel_size=3, stride=1, padding=1)
        self.conv_tcb_offset3 = nn.Conv2d(256, 2*3*3, kernel_size=3, stride=1, padding=1)
        
        self.conv_tcb_modu0 = nn.Conv2d(256, 3*3, kernel_size=3, stride=1, padding=1)
        self.conv_tcb_modu1 = nn.Conv2d(256, 3*3, kernel_size=3, stride=1, padding=1)
        self.conv_tcb_modu2 = nn.Conv2d(256, 3*3, kernel_size=3, stride=1, padding=1)
        self.conv_tcb_modu3 = nn.Conv2d(256, 3*3, kernel_size=3, stride=1, padding=1)
        
        self.tcb_dcn0 = DCNv2(256, 256, kernel_size=3, stride=1, padding=1).cuda()
        self.tcb_dcn1 = DCNv2(256, 256, kernel_size=3, stride=1, padding=1).cuda()
        self.tcb_dcn2 = DCNv2(256, 256, kernel_size=3, stride=1, padding=1).cuda()
        self.tcb_dcn3 = DCNv2(256, 256, kernel_size=3, stride=1, padding=1).cuda()
    ...
    def forward(self, x: torch.Tensor):
        ...
        offset_sources0 = self.conv_sources_offset0(sources[0])
        offset_sources1 = self.conv_sources_offset1(sources[1])
#         offset_sources2 = self.conv_sources_offset2(sources[2])
#         offset_sources3 = self.conv_sources_offset3(sources[3])
        
        modu_sources0 = self.conv_sources_modu0(sources[0])
        modu_sources1 = self.conv_sources_modu1(sources[1])
#         modu_sources2 = self.conv_sources_modu2(sources[2])
#         modu_sources3 = self.conv_sources_modu3(sources[3])
        
        sources[0] = self.sources_dcn0(sources[0], offset_sources0, modu_sources0)
        sources[1] = self.sources_dcn1(sources[1], offset_sources1, modu_sources1)
#         sources[2] = self.sources_dcn2(sources[2], offset_sources2, modu_sources2)
#         sources[3] = self.sources_dcn3(sources[3], offset_sources3, modu_sources3)
        
        arm_loc, arm_conf = self.arm_loc_and_conf(sources)
        
        tcb_source: List[torch.Tensor] = self.tcb_forward(sources)
        offset_tcb0 = self.conv_tcb_offset0(tcb_source[0])
        offset_tcb1 = self.conv_tcb_offset1(tcb_source[1])
#         offset_tcb2 = self.conv_tcb_offset2(tcb_source[2])
#         offset_tcb3 = self.conv_tcb_offset3(tcb_source[3])
        
        modu_tcb0 = self.conv_tcb_modu0(tcb_source[0])
        modu_tcb1 = self.conv_tcb_modu1(tcb_source[1])
#         modu_tcb2 = self.conv_tcb_modu2(tcb_source[2])
#         modu_tcb3 = self.conv_tcb_modu3(tcb_source[3])
        
        tcb_source[0] = self.tcb_dcn0(tcb_source[0], offset_tcb0, modu_tcb0)
        tcb_source[1] = self.tcb_dcn1(tcb_source[1], offset_tcb1, modu_tcb1)
#         tcb_source[2] = self.tcb_dcn2(tcb_source[2], offset_tcb2, modu_tcb2)
#         tcb_source[3] = self.tcb_dcn3(tcb_source[3], offset_tcb3, modu_tcb3)
...
def build_refinedet_deform(phase: str, size: int = 320, num_classes=21):
